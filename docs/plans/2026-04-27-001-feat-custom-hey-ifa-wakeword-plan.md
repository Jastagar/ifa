...
Train an openWakeWord model that detects the single word `"ifa"`, ship the resulting ONNX file inside the repo, and make it the default `IFA_WAKE_MODEL`. This replaces the placeholder `ifa` built-in we shipped in Stage 2 — which works but is the wrong name and uses the prefixed `hey_` form the user has consistently disliked.
...
- **R1.** Detect the spoken word `"ifa"` reliably on the repo owner's voice — target ≥ 9/10 hit rate at conversational distance over an n=10 sample, **non-regression** against `ifa` (which scores 10/10 today). The harder bar — n=10 with 9/10 floor — exists because the n=5 "≥4/5" gate the earlier draft used has no statistical power and could pass a model that's a strict regression in everyday feel.
- **R2.** False-positive rate at conversational volume **≤ 1 unintended wake per 30 minutes** AND ≤ what `ifa` produces today. Both bars matter: an absolute floor (so the system is usable) plus a non-regression check (so the swap doesn't trade naming for usability). The ifa baseline must be measured before declaring R2 met — see Unit 3.
...
- **No automatic *runtime* fallback that masks training failure.** If `ifa` underperforms in everyday use, the user manually flips `IFA_WAKE_MODEL` back to `ifa` (or to `hey ifa` once Stage 3.5 lands). The only automatic fallback is the **missing-file** path (covered in Unit 4) — and even that logs a prominent warning so the divergence is observable, not silent. Distinct from the "underperforms" failure mode.
...
- [ifa/voice/wake_word.py](../../ifa/voice/wake_word.py):48 (`_DEFAULT_MODEL = "ifa"`) — this is what gets swapped to point at the new ONNX when training is verified.
...
- **Make `ifa.onnx` the new `_DEFAULT_MODEL`** in `wake_word.py` — but expressed as a path resolved relative to the package, not a built-in name. Keep `ifa` as a documented fallback option in `.env.example` so users without the trained model (e.g. forks, or if the file is removed) still have a working built-in.
...
- **Verification harness is `scripts/smoke_voice.py record`**, run with `IFA_WAKE_MODEL=ifa/models/ifa.onnx`. This is the same harness we used to validate `ifa` and to debug `hey_jarvis`/`alexa` failures. It saves a WAV and offline-scores per chunk — perfect for "does this model recognize my voice?" without a full run.
...
- Make `ifa.onnx` the new default; keep `ifa` documented as manual fallback.
...
- R1 / R2 verification: n=10 sample with ≥9/10 floor (non-regression vs. ifa); FP-rate gate moved up to Unit 3.
...
.env.example                      (modified — IFA_WAKE_MODEL default updated, ifa documented as manual fallback)
...
- Activity 2 (FP-rate baseline): before measuring `ifa`, measure the **ifa FP rate** at threshold 0.7 / consecutive=2 over a 30-min window of normal use. This is the comparison number for R2.
...
- Record offline max scores across the n=10 hit-rate run. Goal: ≥ 0.8 on at least 9 of 10 attempts (matches R1's "≥9/10 non-regression" against ifa's 10/10).
...
**Patterns to follow:**
- Same harness used to validate `ifa` — see project memory `project_wake_word.md` for the empirical-verification pattern that prevented shipping `alexa` and `hey_jarvis` blindly.

**Test scenarios:** Test expectation: none — operator-driven empirical verification. The output is two recorded numbers (hit-rate at threshold T / consecutive C, and FP-per-30-min) plus a ifa FP baseline for the R2 comparison.
...
- FP rate ≤ 1/30min AND ≤ measured ifa baseline.
- Empirically-found threshold + consecutive values recorded for use in Unit 4's `.env.example` update.

---

- [x] **Unit 4: Make `ifa.onnx` the new default**  *(scaffolding landed in `93f7a24`; final flip + test sweep landed in subsequent commit. `_DEFAULT_MODEL` now resolves to the bundled `ifa/models/ifa.onnx` via `pathlib.Path(__file__).resolve().parent.parent / "models" / "ifa.onnx"`. test_wake_word.py predict-mock keys flipped from `ifa` → `ifa` across detect tests; new `test_init_resolves_default_to_bundled_ifa_path_no_download` asserts the path-based default; new `BundledOnnxIntegrityTests` class loads the real `.onnx` via onnxruntime to guard against committing a corrupted blob. 187/187 tests pass.)*
...
- **Move the missing-file fallback into `_resolve_model_spec` (or its caller)**, NOT module-import time. Per-listener-construction check ensures the fallback re-evaluates if the bundled file is somehow deleted between imports, and lets tests exercise the fallback via `os.path.exists` patching without re-importing the module. Logic: if the resolved spec looks like a path (`os.path.exists`-style probe), and the path doesn't exist, fall back to `"ifa"` (built-in name) so the listener still boots.
...
- Edge case: with `IFA_WAKE_MODEL=ifa` set, the listener uses the built-in name as before — backwards compatibility for users who explicitly opt out.
- Edge case: bundled .onnx file is loadable via `onnxruntime.InferenceSession` (file integrity guard for the committed binary).
- Error path: with the bundled file deleted (simulated via patch on `os.path.exists`) and no env override, `_DEFAULT_MODEL` falls back to `"ifa"` and the listener constructs cleanly via the built-in name path.
...
- Update: `~/.claude/projects/-Users-jastagarbrar-Projects-ifa/memory/project_wake_word.md` — flip from "ifa for v1" to "ifa for v1 (single word, no prefix); ifa retained as documented manual fallback; hey_ifa documented as Stage 3.5 retreat path if FP rate becomes unacceptable".
...
- Update README's "wake word" sentence from "ifa" to "ifa".
...
- **Error propagation:** If the bundled .onnx is somehow missing or corrupted, the existing `WakeWordInitError` path catches it and prints an actionable message. The fallback to `"ifa"` (Unit 4) makes this strictly better than today.
...
| **Missing-file fallback to `ifa` is silent and creates an identity-failure mode** (user gets a working wake word with the wrong name and no obvious indication) | Medium | Medium | Fallback now logs a prominent WARNING and the launcher startup line distinguishes `wake=ifa (bundled)` from `wake=ifa (FALLBACK — bundled ifa.onnx not found)`. Behavior is observable rather than invisible. |

## Documentation / Operational Notes

- README's voice-mode section (currently: *"The wake word is `ifa` for v1..."*) updates to `ifa` (single word, no prefix).
- `.env.example`'s `IFA_WAKE_MODEL` block updates to show the bundled path as default and `ifa` (built-in) as a fallback.
